...
suite(
  function (env) {
    describe("Leave History", function () {
      let driver;
      ...
      it("opened leave histories page correctly", async function () {
        ...
        const openAccountMenu = await driver.findElement(
          By.id("leave_histories")
        );
        await openAccountMenu.click();

        await driver.sleep(2000);
        const openAccountUrl =
          (await process.env.REACT_APP_BASE_URL) + "Personnel/leave_histories";
