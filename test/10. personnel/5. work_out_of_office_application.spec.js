...
suite(
  function (env) {
    describe("Working Out Of Office application", function () {
      let driver;
      ...
      it("opened working out of office application page correctly", async function () {
        ...
        const openAccountMenu = await driver.findElement(
          By.id("wooo_application")
        );
        await openAccountMenu.click();

        await driver.sleep(2000);
        const openAccountUrl =
          (await process.env.REACT_APP_BASE_URL) + "Personnel/wooo_application";
